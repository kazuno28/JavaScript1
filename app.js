...
// <<変数宣言の使い分け>>
// ・変数宣言の際に意識すべきことは、「スコープ」と「再宣言」。
// 　　→できる限り、変数のスコープは使用される箇所で局地的であることが望ましい。
//
// ・スコープが広いことで、意図しない参照からエラーが発生したり、期待通りの結果が得られないリスクが高くなる。
// 　　→修正作業にたくさんの時間が取られるので、varを使用する場面というのはほとんどなくなっている。
//　　 →昨今は、letとconstを変数宣言に使用していくことになる。
//
// <letとconstの使い分けについて>
// ・letは再代入が可能、constが再代入が不可能。
// 　　→constで宣言した変数に値を格納した場合、新たに値を代入することはできない。
// 　　→基本的に、新たに値を代入する必要のないものに関してはconstで宣言していくことになる。
//
// ・letは再代入をする場面において、使用すべきものとなる。
// (ex)
/* for (let i = 0; i < 10; i++) {
   console.log(i);
   } */
// ↑　0~9までをループによってコンソールに出力するコード
// ・この場合、iをletで宣言する理由はループするたびにiに再代入を行うため。
//
// ・constは定数として扱う変数の宣言に使う、といった内容で記載されているものもあるが、
// 　constのもつ特徴として定数的に扱えるというのが正しい表現となる。
//　　 →適切なスコープで変数宣言を行っていくことで、意図しないエラーや結果を招く可能性は低くなる。